    ...
    class IpoRows < Snabberb::Component
      ...
      def render_first_ipo(ipo_row, number)
        ...
        if @step.respond_to?(:general_input_renderings_ipo_row)
          puts "Responds to general_input_renderings_ipo_row"
          index = number - 1
          renderings = @step.general_input_renderings_ipo_row(@current_entity, first_company, index)
          renderings.each do |(choice, description)|
            puts "Rendering #{choice} - #{description}"
            inputs.concat(render_general_input(first_company, choice, description))
          end
        end
        children = []
      ...
      def render_general_input(company, choice, description)
          action = lambda do
